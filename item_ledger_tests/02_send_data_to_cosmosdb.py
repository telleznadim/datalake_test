...
def readCsvBcSendCosmosDB(file_name, container, df_from_cosmosdb):
    df = pd.read_csv(
        f'C:/Users/eviadmin/Documents/Datawarehouse/python_scripts/DataLake/test/item_ledger_tests/files/from_bc/{file_name}', compression="gzip")
    print(df)
...
def main():
    ...
    endpoints = {"ItemLedgerTest": "Item_Ledger_Entries_38_DW"}
    # endpoints = {"Items": "Items_31_DW"}
    ...
    for endpoint in endpoints:
        ...
        for company_short in companies:
            ...
            if (endpoints[endpoint] == "Item_Ledger_Entries_38_DW"):
                df_from_cosmosdb = query_items_by_bu(
                    container, company_short, "Entry_No")
                readCsvBcSendCosmosDB(file_name, container, df_from_cosmosdb)
            else:
